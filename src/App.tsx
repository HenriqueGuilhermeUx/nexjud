...
import LiveDossier from "@/pages/LiveDossier"
import OfficeIntelligence from "@/pages/OfficeIntelligence"
...
import PrecedentIntelligence from "@/pages/PrecedentIntelligence"
import CNJProcesses from "@/pages/CNJProcesses"
import PlanGate from "@/components/PlanGate"

function ProtectedRoute({ children }: { children: React.ReactNode }) {
  const { user, loading } = useAuth()
  if (loading) return <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#6366f1]" /></div>
...
export default function App() {
  return <AuthProvider><BrowserRouter><Routes>
    ...
    <Route path="/dashboard" element={<ProtectedRoute><Dashboard /></ProtectedRoute>}>
      <Route index element={<HomeDashboard />} /><Route path="ai-copilot" element={<AICopilot />} /><Route path="ai-copilot-history" element={<AICopilotHistory />} /><Route path="legal-chat" element={<LegalChat />} /><Route path="knowledge-base" element={<KnowledgeBase />} /><Route path="legal-memory" element={<LegalMemory />} /><Route path="legal-cases" element={<LegalCases />} /><Route path="live-dossier" element={<LiveDossier />} /><Route path="office-intelligence" element={<OfficeIntelligence />} /><Route path="onboarding" element={<Onboarding />} /><Route path="process-check" element={<ProcessCheck />} /><Route path="predictive" element={<PredictiveAI />} /><Route path="jurisprudence" element={<Jurisprudence />} /><Route path="red-team" element={<RedTeam />} /><Route path="reports" element={<StrategicReport />} /><Route path="history" element={<History />} /><Route path="red-team-simulator" element={<RedTeamSimulator />} /><Route path="draft-generator" element={<DraftGenerator />} /><Route path="draft-history" element={<DraftHistory />} /><Route path="judge-simulator" element={<JudgeSimulator />} /><Route path="judge-history" element={<JudgeHistory />} /><Route path="process-portfolio" element={<ProcessPortfolio />} /><Route path="enterprise-command-center" element={<EnterpriseCommandCenter />} /><Route path="chief-legal-officer" element={<ChiefLegalOfficer />} /><Route path="jurisprudence-library" element={<JurisprudenceLibrary />} /><Route path="precedents" element={<Precedents />} /><Route path="precedent-intelligence" element={<PrecedentIntelligence />} /><Route path="cnj-processes" element={<CNJProcesses />} />
      <Route path="legal-intelligence-engine" element={<PlanGate requiredPlan="intelligence"><LegalIntelligenceEngine /></PlanGate>} /><Route path="legal-intelligence-history" element={<PlanGate requiredPlan="intelligence"><LegalIntelligenceHistory /></PlanGate>} /><Route path="litigation-strategy" element={<PlanGate requiredPlan="enterprise"><LitigationStrategyCenter /></PlanGate>} /><Route path="war-room" element={<WarRoomCenter />} /><Route path="partner-council" element={<PartnerCouncilCenter />} /><Route path="one-click-actions" element={<OneClickActionsCenter />} /><Route path="opponent-database" element={<OpponentDatabaseCenter />} /><Route path="tribunal-heatmap" element={<TribunalHeatmapCenter />} />
    </Route>
